...
import UIKit
import RealmSwift
class RecentPatientViewController: UIViewController,UITableViewDelegate,UITableViewDataSource, UITabBarControllerDelegate,UIGestureRecognizerDelegate {
    var ui = UITable()
...
//        })
    ...
    override func viewDidLoad() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(scrollTop), name: NSNotification.Name("recentTabbar"), object: nil)
    ...
    func loadUpdatedRecent(){
        ...
        if self.patients.count > 0 {
            self.api.listUpdatedRecents(sessionid: self.backSystem.getSessionid(), updatetime: self.helper.dateToServer(date: self.patients[0].lastfuupdatetime) , success: {(success) in
                self.back.loadPatientData(dic: success)
                self.patients = self.back.sortPatientByFuDate()
                self.tableView.reloadData()
                self.vw_new_record.isHidden = true
